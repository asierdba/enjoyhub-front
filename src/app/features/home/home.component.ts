...
export class HomeComponent {
  ...
  loadByEmotion(emotion: Emotion): void {
    this.currentIndex.set(0);
    this.contents.set([]);
    this.contentsLoading.set(true);
  ...
  filterContent(list: Content[]): Content[] {
    const discarded = this.discardedIds();
    const inList    = new Set(this.listItems().map(index => index.contentId));
    return [...list]
  ...
  discard(): void {
    ...
    if (!content || this.discardLoading()) {
      return;
    }

    this.discardLoading.set(true);
    this.discardedIds.update(solo => new Set([...solo, content.contentId]));
  ...
  doAddToList(): void {
    ...
    if (!listId || !content || this.addLoading()) {
      return;
    } 
